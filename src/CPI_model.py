...
class Net(nn.Module):
    def __init__(self, init_atom_features, init_bond_features, init_word_features, params):
        ...
        self.W_out = nn.Linear(self.hidden_size2*self.hidden_size2*2, 1) # Output layer
        ...
        self.atomwise_pooling = False
        self.contrast_learner = SoftNearestNeighbor(tau=2)
    ...
    def GraphConv_module(self, batch_size, vertex_mask, vertex, edge, atom_adj, bond_adj, nbs_mask):
        ...
        for GWM_iter in range(self.GNN_depth):
            ...
            for k in range(self.k_head):
                ...
                if k == 0: m_main_to_super = k_main_to_super
                else: m_main_to_super = torch.cat([m_main_to_super, k_main_to_super], dim=-1)
    ...
    #def make_contrastive(self, atomwise_features, resiwise_features, atomresi_graph, pairwise_pred):
    def make_contrastive(self, atomwise_features, resiwise_features, atomresi_graph):
        print(atomwise_features.shape)
        print(resiwise_features.shape)
        print(atomresi_graph.shape)
        positive_edges = torch.nonzero(atomresi_graph, as_tuple=True)
        positive_resis = resiwise_features[positive_edges[1], :]
        pred_edges = torch.nonzero(atomresi_graph+1, as_tuple=True)  # all edges

        if self.atomwise_pooling:
            try:
                pred_resis = resiwise_features[pred_edges[1], :]
                contrastive_loss = self.contrast_learner(atomwise_features, positive_resis, atomwise_features, pred_resis)
            except: traceback.format_exc()
        else:
            positive_atoms = atomwise_features[positive_edges[0], :]
            pred_atoms = atomwise_features[pred_edges[0], :]
            pred_resis = resiwise_features[pred_edges[1], :]
            contrastive_loss = self.contrast_learner(positive_atoms, positive_resis, pred_atoms, pred_resis)
    ...
    def forward(self, vertex_mask, vertex, edge, atom_adj, bond_adj, nbs_mask, seq_mask, sequence):
        ...
        #################### soft nearest neighbor loss #################
        aux_loss_list = []
        pw_pred_nonmask = [pw[:torch.nonzero(pw)[-1][0].item()+1, :torch.nonzero(pw)[-1][1].item()+1] for pw in pairwise_pred]

        for idx, pw_pred in enumerate(pw_pred_nonmask):
            comp_size = pw_pred_nonmask[idx].shape[0]
            prot_size = pw_pred_nonmask[idx].shape[1]
            atomwise_features = atom_feature[idx][:comp_size+1]
            resiwise_features = prot_feature[idx][:prot_size+1]
            aux_loss_list.append(self.make_contrastive(atomwise_features, resiwise_features, pw_pred))
            print(idx)
        aux_loss = sum(aux_loss_list) / len(aux_loss_list)
        #################### soft nearest neighbor loss #################

        return affinity_pred, pairwise_pred, aux_loss


class SoftNearestNeighbor(nn.Module):
    def __init__(self, tau=2.0):
        super(SoftNearestNeighbor, self).__init__()
        self.tau = tau

    def forward(self, anchor, positives, pred_anchor, pred_target):
        positive_scores = (-1*((anchor-positives).pow(2).sum(1) /
                           self.tau)).exp().sum()
        target_scores = (-1*((pred_anchor -
                         pred_target).pow(2).sum(1)/self.tau)).exp().sum()

        return -1*(positive_scores / target_scores).log()
