...
def train_and_eval(train_data, valid_data, test_data, params, batch_size=32, num_epoch=30):
    ...
    )), lr=0.0005, weight_decay=0, amsgrad=True)
    ...
    for epoch in range(num_epoch):
        ...
        for i in range(int(len(train_data[0])/batch_size)):
            ...
            input_vertex, input_edge, input_atom_adj, input_bond_adj, input_num_nbs, input_seq, affinity_label, pairwise_mask, pairwise_label = \
                [train_data[data_idx][shuffle_index[i *
                                                    batch_size:(i+1)*batch_size]] for data_idx in range(9)]
            ...
            optimizer.zero_grad()
            affinity_pred, pairwise_pred, aux_loss = net(vertex_mask, vertex, edge, atom_adj, bond_adj, nbs_mask, seq_mask, sequence)
            ...
            loss = loss_aff + 0.1*loss_pairwise + aux_loss
...
def test(net, test_data, batch_size):
    ...
    for i in range(int(math.ceil(len(test_data[0])/float(batch_size)))):
        input_vertex, input_edge, input_atom_adj, input_bond_adj, input_num_nbs, input_seq, aff_label, pairwise_mask, pairwise_label = \
            [test_data[data_idx][i*batch_size:(i+1)*batch_size]
             for data_idx in range(9)]
        ...
        for j in range(len(pairwise_mask)):
            if pairwise_mask[j]:
                ...
                pairwise_pred_i = pairwise_pred[j, :num_vertex, :num_residue].cpu(
                ).detach().numpy().reshape(-1)
                pairwise_label_i = pairwise_label[j].reshape(-1)
                pairwise_auc_list.append(roc_auc_score(
                    pairwise_label_i, pairwise_pred_i))
...
if __name__ == "__main__":
    ...
    for a_rep in range(n_rep):
        ...
        np.save('MONN_rep_all_list_'+measure+'_'+setting +
                '_thre'+str(clu_thre), rep_all_list)
